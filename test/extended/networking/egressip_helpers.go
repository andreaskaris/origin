...
// sendEgressIPProbesAndCheckOutput sends requests with a unique search string from the prober pod. It then
...
func sendEgressIPProbesAndCheckOutput(
	oc *exutil.CLI, proberPod *corev1.Pod, routeName, targetProtocol, targetHost string, targetPort,
	probeCount, expectedHits int, egressIPSet map[string]string, logScanMaxTries int) (bool, error) {
	...
	if targetProtocol != "http" && targetProtocol != "udp" {
		return false, fmt.Errorf("sendProbesToHostPort supports only 'http' and 'udp' protocols")
	}
	...
	// UDP or HTTP targetHost:targetPort and to run the clientip request.
	request := fmt.Sprintf("http://%s/dial?protocol=%s&host=%s&port=%d&request=%s", routeName, targetProtocol, targetHost, targetPort, requestStr)
...
func addIPAddressToHost(oc *exutil.CLI, host, intf, ip string) error {
	cmd := fmt.Sprintf("ip address add %s/32 dev %s", ip, intf)
	_, err := oc.AsAdmin().Run("debug").Args("node/"+host, "--", "/bin/bash", "-c", cmd).Output()
	return err
}

// removeIPAddressFromHost adds an IP address to the host's interface.
func removeIPAddressFromHost(oc *exutil.CLI, host, intf, ip string) error {
